...
import { TMDB_CAST_URL } from 'constants';

const TitleCast = ({ cast }) => {
  if (!cast || cast.length === 0) return null;
  return (
    cast.map((person) => (
      <div className="flex flex-col mt-1 mr-0 ml-0 mb-2 px-2" key={`cast-person-${person.id}`}>
        <img src={`${TMDB_CAST_URL}${person.profile_path}`} className="rounded-full h-20 w-20 mx-auto" alt={`cast-person-${person.id}`} />
